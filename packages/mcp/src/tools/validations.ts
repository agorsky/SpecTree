...
export function registerValidationTools(server: McpServer): void {
  // ==========================================================================
  // spectree__manage_validations - COMPOSITE TOOL
  // ==========================================================================
  server.registerTool(
    "spectree__manage_validations",
    {
      description:
        "Manage validation checks for tasks. This composite tool provides 7 actions:\n\n" +
        "**add**: Add a validation check (command, file_exists, file_contains, test_passes, manual)\n" +
        "**list**: List all validation checks with their status (pending, passed, failed)\n" +
        "**run**: Run a single validation check and update its status\n" +
        "**run_all**: Run all validation checks and return summary\n" +
        "**mark_manual**: Mark a manual validation check as validated\n" +
        "**remove**: Remove a validation check from a task\n" +
        "**reset**: Reset all validation checks to pending status\n\n" +
        "Use this to define, execute, and track acceptance criteria for tasks.",
      inputSchema: z.discriminatedUnion("action", [
        // Add validation
        z.object({
          action: z.literal("add"),
          taskId: z.string().describe("Task identifier (UUID or e.g., 'COM-123-1')"),
          type: z.enum(validationCheckTypes).describe("Validation check type"),
          description: z.string().min(1).max(500).describe("What this check validates"),
          command: z.string().max(1000).optional().describe("Shell command (for 'command' type)"),
          expectedExitCode: z.number().int().min(0).max(255).optional().describe("Expected exit code (default: 0)"),
          timeoutMs: z.number().int().min(1000).max(300000).optional().describe("Timeout in ms (default: 30000)"),
          filePath: z.string().max(500).optional().describe("File path (for 'file_exists' or 'file_contains')"),
          searchPattern: z.string().max(1000).optional().describe("Regex pattern (for 'file_contains')"),
          testCommand: z.string().max(1000).optional().describe("Test command (for 'test_passes')"),
        }),
        // List validations
        z.object({
        ...
        z.object({
          ...
          taskId: z.string().describe("Task identifier (UUID or e.g., 'COM-123-1')"),
          checkId: z.string().uuid().describe("Validation check ID"),
          workingDirectory: z.string().max(1000).optional().describe("Working directory for command execution"),
        }),
        // Run all validations
        z.object({
          action: z.literal("run_all"),
          taskId: z.string().describe("Task identifier (UUID or e.g., 'COM-123-1')"),
        ...
        z.object({
          action: z.literal("mark_manual"),
          taskId: z.string().describe("Task identifier (UUID or e.g., 'COM-123-1')"),
          checkId: z.string().uuid().describe("Manual validation check ID"),
          notes: z.string().max(1000).optional().describe("Notes about manual verification"),
        }),
        // Remove validation
        z.object({
          action: z.literal("remove"),
          taskId: z.string().describe("Task identifier (UUID or e.g., 'COM-123-1')"),
          checkId: z.string().uuid().describe("Validation check ID to remove"),
        }),
        // Reset validations
        z.object({
    ...
    async (input) => {
      try {
        ...
        // Resolve task identifier to UUID (all actions need this)
        const { data: task } = await apiClient.getTask(input.taskId);

        switch (input.action) {
          case "add": {
            // Build add input conditionally to avoid undefined values
            const addInput: {
  ...
  server.registerTool(
    ...
    {
      description:
        "⚠️ DEPRECATED: Prefer spectree__manage_validations with action='add'. " +
  ...
  server.registerTool(
    ...
    {
      description:
        "⚠️ DEPRECATED: Prefer spectree__manage_validations with action='list'. " +
  ...
  server.registerTool(
    ...
    {
      description:
        "⚠️ DEPRECATED: Prefer spectree__manage_validations with action='run'. " +
  ...
  server.registerTool(
    ...
    {
      description:
        "⚠️ DEPRECATED: Prefer spectree__manage_validations with action='run_all'. " +
  ...
  server.registerTool(
    ...
    {
      description:
        "⚠️ DEPRECATED: Prefer spectree__manage_validations with action='mark_manual'. " +
  ...
  server.registerTool(
    ...
    {
      description:
        "⚠️ DEPRECATED: Prefer spectree__manage_validations with action='remove'. " +
  ...
  server.registerTool(
    ...
    {
      description:
        "⚠️ DEPRECATED: Prefer spectree__manage_validations with action='reset'. " +
